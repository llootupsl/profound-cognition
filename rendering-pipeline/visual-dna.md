...
2. **熔断可控偏离**: 在熔断机制允许的范围内（质量驱动终止：连续 2 次重试分数提升 < 1 分即终止，无硬性重试次数上限，符合 EXHAUST 模式四大铁律），允许对五维门禁未满分维度进行定向修复；已满分维度保持不动，避免"修一处坏一处"的回退风险。质量驱动终止后质量保持为最高分方案，不再追求满分。
...
> **v2.0 重构说明**: PaperBanana 原依赖 Google 专有 API（Nano Banana Pro / Gemini 3 Pro Image），现已重构为**纯代码生成**方式。visual_dna 字段注入内联 SVG / Canvas / Typst draw 代码生成器，而非 AI 生图 API。

**消费 visual_dna 字段**（源自 DLP `design_tokens`，可追溯至 DLP 锚定实体）：`color_scheme`（→学术插图配色）、`font_scheme`（→插图标注字体）、`line_style`（→插图线条风格）

**对接映射**：

| visual_dna 字段 | 代码生成配置项 | 映射规则 |
|----------------|------------------|---------|
| `--color-primary` | 插图主色调 | 关键元素着色（SVG fill/stroke） |
| `--color-secondary` | 插图辅色调 | 辅助元素着色（SVG fill/stroke） |
| `--color-text` | 标注文字色 | 图注/标签（SVG text fill） |
| `--color-border` | 插图边框/连线色 | 连接线/边框（SVG stroke） |
| 正文字体 | 标注字体 | 图内文字（SVG font-family） |
| 流程图连线 1.5px | 连线粗细 | 示意图连线（SVG stroke-width） |

**决策规则**：顶刊级学术插图（机制图/信号通路/系统架构图）使用**内联 SVG 代码生成**（PaperBanana 方法论·代码生成）；手绘风格示意图使用 excalidraw。
...
**决策规则**：概念示意图/白板风格/非正式说明图使用 excalidraw；正式学术插图使用**内联 SVG（PaperBanana 方法论·代码生成）**。
...
> **v2.0 重构说明**: PaperBanana 原依赖 Google 专有 API（Nano Banana Pro / Gemini 3 Pro Image）生成插图，违反"代码生成优先"原则。现已重构为**纯代码生成**方式——通过内联 SVG / Canvas / Typst draw 代码生成顶刊级学术插图，完全不依赖任何 AI 生图 API。详见 [illustration-generator.md §0 核心铁律](../output/illustration-generator.md)。

**方法论原理**：PaperBanana 方法论是渲染管道中面向顶刊学术插图的**代码生成**引擎，采用语义描述-代码生成-学术规范范式。其核心设计原理是"文字即插图"：通过自然语言描述插图内容（如"Wnt信号通路示意图"、"系统架构图"），由 LLM 直接书写内联 SVG / Canvas / Typst draw 代码生成符合学术规范的顶刊级插图。与 ECharts/Plotly 的数据驱动图表不同，PaperBanana 方法论生成的是概念性插图（机制图/信号通路/架构图），而非数据图表。

**执行步骤**：
1. 插图需求分析：确定插图类型（机制图/信号通路/架构图/流程示意图）
2. 语义描述编写：用自然语言描述插图内容和元素关系
3. visual_dna 注入：从 visual_dna 读取学术配色/字体/线宽
4. 代码生成：根据语义描述直接书写内联 SVG / Canvas / Typst draw 代码（禁止调用任何 AI 生图 API）
5. 学术规范校验：检查插图是否符合学术出版规范
6. 格式导出：代码直接渲染为 SVG/矢量格式

**决策规则**：

| 条件 | 决策 |
|------|------|
| 顶刊级学术插图（机制图/信号通路） | 使用内联 SVG（PaperBanana 方法论·代码生成） |
...
**方法论原理**：excalidraw 是渲染管道中的手绘风格示意图引擎，采用手绘渲染-白板风格范式。其核心设计原理是"手绘即沟通"：通过模拟手绘线条的不规则性（抖动/粗细变化/圆角端点），创造非正式、亲和力强的视觉风格，适合概念解释、白板讨论和快速原型。excalidraw 与 PaperBanana 方法论的区别在于风格定位——excalidraw 是"白板讨论级"，PaperBanana 方法论（v2.0 代码生成版）是"顶刊出版级"（通过内联 SVG/Canvas/Typst draw 代码生成）。
...
| 正式学术插图 | 使用内联 SVG（PaperBanana 方法论·代码生成） |
| 流程图/时序图 | 使用 Mermaid |
