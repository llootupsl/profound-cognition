...
> **强制规则**: DLP 检索器在 Taste-Skill 生成 `visual_dna` 前必须命中一个 DLP，未命中时格式适配到 `DLP-nature`（学术严谨默认锚点）。
...
| 校验项 | 校验规则 | 失败处理 |
|--------|---------|---------|
| 配色可追溯性 | visual_dna.color_scheme 的每个色值必须能在 DLP color_palette 中找到源色或派生链 | 拒绝注入，格式适配到 DLP 原始色值 |
...
| 圆角阴影合规性 | visual_dna 的圆角/阴影不得超出 DLP radius_shadow 的定义（DLP 为 0px 时禁止圆角） | 强制格式适配到 DLP 值 |
...
## 八、DLP 覆盖度矩阵与补全（R9-04）

> **目的**：检查 16 个 DLP 在 `output_type × target_audience` 矩阵上的覆盖度，补全未覆盖组合的格式适配 DLP，确保任何输入组合都能命中至少一个 DLP。

### 8.1 覆盖度矩阵

下表展示所有 `output_type × target_audience` 组合的 DLP 覆盖情况：

| output_type \ target_audience | academic | professional | general | youth |
|------------------------------|----------|-------------|---------|-------|
| **research_report** | ✅ DLP-nature / DLP-science / DLP-ieee / DLP-springer / DLP-economist-chart / DLP-scienceplots / DLP-nature-figure | ✅ DLP-plotivy | ⚠️ 补全：格式适配 DLP-economist | ⚠️ 补全：格式适配 DLP-ted |
| **wechat_article** | ⚠️ 补全：格式适配 DLP-nature | ✅ DLP-linear / DLP-stripe-press | ✅ DLP-aesop / DLP-gov-uk / DLP-economist / DLP-newyorker / DLP-kami | ⚠️ 补全：格式适配 DLP-ted |
| **course_material** | ⚠️ 补全：格式适配 DLP-ieee | ⚠️ 补全：格式适配 DLP-linear | ⚠️ 补全：格式适配 DLP-ted | ✅ DLP-ted |

> **图例**: ✅ 有直接命中 DLP | ⚠️ 无直接命中，使用格式适配 DLP

### 8.2 未覆盖组合的格式适配规则

对于矩阵中标记为 ⚠️ 的组合，DLP 检索器按以下格式适配规则命中 DLP：

| output_type | target_audience | 格式适配命中 DLP | 格式适配理由 |
|-------------|-----------------|-------------|---------|
| research_report | general | DLP-economist | general 受众的研究报告偏向新闻杂志风格，Economist 排版兼顾严肃与可读性 |
| research_report | youth | DLP-ted | youth 受众的研究报告需要大字号与留白驱动，TED 风格降低阅读门槛 |
| wechat_article | academic | DLP-nature | academic 受众的公众号文章需要学术严谨风格，Nature 是学术默认锚点 |
| wechat_article | youth | DLP-ted | youth 受众的公众号文章需要演示级大字号与极简风格 |
| course_material | academic | DLP-ieee | academic 受众的课程材料需要工程严谨风格，IEEE 双栏排版适合教学内容 |
| course_material | professional | DLP-linear | professional 受众的课程材料需要界面品牌风格，Linear 适合技术培训 |
| course_material | general | DLP-ted | general 受众的课程材料需要演示风格，TED 大字号适合通识教学 |

### 8.3 覆盖度统计

| 指标 | 数值 |
|------|------|
| output_type × target_audience 组合总数 | 12 |
| 有直接命中 DLP 的组合数 | 5 |
| 使用格式适配 DLP 的组合数 | 7 |
| DLP 覆盖率（直接命中） | 41.7% |
| DLP 覆盖率（含格式适配） | 100% |
...
DLP 检索器在 §4.2 检索算法的 Step 2（族内 DLP 匹配）中，若匹配度为 0，则按本节 §8.2 的格式适配规则命中格式适配 DLP。格式适配命中时，`dlp_retriever_output.exhaust_retry_used` 设为 `true`，`exhaust_retry_reason` 设为 `"no_direct_match_format_adaptation_to_{dlp_name}"`。

> **R2-01 例外条款声明**：DLP 检索器的格式适配不视为降级——从精确匹配 DLP 到默认 DLP 是视觉风格变化，内容完整保留。

### 8.5 未来 DLP 扩展建议

为提升直接命中率，建议未来新增以下 DLP：

| 建议 DLP | 族 | 锚定实体 | 覆盖组合 | 优先级 |
|---------|-----|---------|---------|--------|
| DLP-coursera | interface-brand | Coursera 课程界面 | course_material × general | 中 |
| DLP-medium | publication-typesetting | Medium 文章版式 | wechat_article × general | 中 |
| DLP-khan-academy | interface-brand | Khan Academy 教学界面 | course_material × youth | 低 |

> 新增 DLP 须遵循 §6.1 新增 DLP 流程，并在本矩阵中更新覆盖度。
