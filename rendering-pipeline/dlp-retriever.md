...
| §5 | 科学深度层（8 模块） | 讨论层 — 科学验证与深度讨论 | 目标受众推断 |
...
自定义 DLP 检索扩展 DLP 检索器的能力，使其在 16 个内置 DLP 之外，能够扫描并检索用户在 `rendering-pipeline/user-dlps/` 目录下创建的自定义 DLP。自定义 DLP 与内置 DLP **平等参与打分**，遵循"用户自定义优先于内置"的原则——当自定义 DLP 与内置 DLP 得分相同时，优先选择自定义 DLP。

### 11.2 自定义 DLP 候选池扫描算法

```python
# ============================================================
# DLP 检索器 — 自定义 DLP 候选池扫描算法
# 输入: rendering-pipeline/user-dlps/ 目录路径
# 输出: custom_dlp_pool = [custom_dlp, ...]（含 12 字段完整定义）
# ============================================================

function scan_custom_dlp_pool(user_dlps_dir="rendering-pipeline/user-dlps/"):
    """
    扫描 user-dlps/ 目录下所有 .md 文件，解析 YAML frontmatter，
    构建自定义 DLP 候选池
    """

    custom_dlp_pool = []

    # Step 1: 列出目录下所有 .md 文件（排除 README.md）
    md_files = list_files(user_dlps_dir, pattern="*.md")
    md_files = [f for f in md_files if f.name != "README.md"]

    # Step 2: 逐个解析 YAML frontmatter
    for md_file in md_files:
        try:
            # 读取文件内容
            content = read_file(md_file)

            # 提取 YAML frontmatter（--- 与 --- 之间的内容）
            frontmatter = extract_yaml_frontmatter(content)

            # 验证 12 字段完整性
            validation = validate_dlp_frontmatter(frontmatter)

            if validation.is_valid:
                custom_dlp = parse_dlp_from_frontmatter(frontmatter)
                custom_dlp["source"] = "user-defined"  # 标记来源
                custom_dlp["file_path"] = md_file.path
                custom_dlp_pool.append(custom_dlp)
            else:
                # 记录无效 DLP，但不中断扫描
                log_invalid_dlp(md_file, validation.errors)

        except Exception as e:
            log_parse_error(md_file, e)
            continue

    # Step 3: 返回自定义 DLP 候选池
    return custom_dlp_pool


function validate_dlp_frontmatter(frontmatter):
    """
    验证自定义 DLP 的 YAML frontmatter 是否包含完整 12 字段
    """
    REQUIRED_FIELDS = [
        "name", "anchor", "family",
        "color_palette", "typography_scale", "font_stack",
        "font_weight_pairing", "spacing_system", "grid_system",
        "radius_shadow", "motion_curve", "applicable_scenarios"
    ]
    ...
    # 额外验证：applicable_scenarios 必须为非空列表
    if "applicable_scenarios" in frontmatter:
        if not isinstance(frontmatter["applicable_scenarios"], list) or \
           len(frontmatter["applicable_scenarios"]) == 0:
            errors.append("applicable_scenarios 必须为非空列表")

    # 额外验证：family 必须为 4 族之一或自定义族名
    VALID_FAMILIES = [
        "academic-journal", "interface-brand",
        "publication-typesetting", "data-visualization"
    ]
    if "family" in frontmatter:
        # 允许自定义族名（如 "custom-brand"），但需以 "custom-" 前缀标识
        if frontmatter["family"] not in VALID_FAMILIES:
            if not frontmatter["family"].startswith("custom-"):
                errors.append(
                    f"自定义族名必须以 'custom-' 前缀标识，当前值: {frontmatter['family']}"
                )
...
### 11.3 自定义 DLP 与内置 DLP 平等打分规则

自定义 DLP 与 16 个内置 DLP 在打分阶段**平等参与**，遵循以下规则：

| 规则编号 | 规则内容 | 说明 |
|---------|---------|------|
| C-01 | **同族优先合并** | 自定义 DLP 的 `family` 若为 4 族之一，则与该族内置 DLP 一起参与族内打分 |
| C-02 | **自定义族独立打分** | 自定义 DLP 的 `family` 若为 `custom-*`，则作为独立族参与打分，与 4 内置族平等竞争 |
| C-03 | **得分相同时自定义优先** | 当自定义 DLP 与内置 DLP 得分相同时，优先选择自定义 DLP（反映用户显式偏好） |
| C-04 | **场景标签平等匹配** | 自定义 DLP 的 `applicable_scenarios` 与内置 DLP 使用相同的匹配算法（完全匹配 +1.0 / 部分匹配 +0.5 / 不匹配 0） |
| C-05 | **质量保持不降级** | 自定义 DLP 不可用时，回退到内置 DLP 质量保持策略（§七），不因自定义 DLP 缺失而降低检索质量 |
| C-06 | **ASR 硬门验证前置** | 自定义 DLP 在入库前必须通过 ASR 硬门验证（详见 `output/dlp-templates/DLP-template.md` 的验证清单），未通过验证的自定义 DLP 不进入候选池 |

### 11.4 自定义 DLP 场景标签匹配算法

```python
# ============================================================
# DLP 检索器 — 自定义 DLP 场景标签匹配算法
# 输入: custom_dlp_pool + semantic_signals + task_type_mapping
# 输出: scored_custom_dlps = [{dlp_name, score, match_details, source}, ...]
# ============================================================

function score_custom_dlps(custom_dlp_pool, semantic_signals, task_type_mapping):
    """
    对自定义 DLP 候选池中的每个 DLP 按场景标签匹配度打分
    打分算法与内置 DLP 完全一致（复用 score_single_dlp 函数）
    """

    # 构建匹配关键词集（与内置 DLP 打分使用相同的关键词集）
    match_keywords = build_match_keywords(semantic_signals, task_type_mapping)

    scored_custom_dlps = []
    for custom_dlp in custom_dlp_pool:
        # 复用内置 DLP 的打分函数，确保打分逻辑一致
        score, match_details = score_single_dlp(custom_dlp, match_keywords)

        scored_custom_dlps.append({
            "dlp_name": custom_dlp["name"],
            "dlp_family": custom_dlp["family"],
            "score": score,
            "match_details": match_details,
            "applicable_scenarios": custom_dlp["applicable_scenarios"],
            "source": "user-defined",  # 标记来源为用户自定义
            "file_path": custom_dlp.get("file_path")
        })

    # 按分数降序排序
    scored_custom_dlps.sort(key=lambda x: x["score"], reverse=True)

    return scored_custom_dlps


function merge_and_rank_dlps(scored_builtin_dlps, scored_custom_dlps):
    """
    合并内置 DLP 与自定义 DLP 的打分结果，统一排序
    遵循规则 C-03：得分相同时自定义 DLP 优先
    """

    # 合并两个列表
    merged_dlps = scored_builtin_dlps + scored_custom_dlps

    # 排序规则：
    # 1. 按分数降序
    # 2. 分数相同时，自定义 DLP（source="user-defined"）优先于内置 DLP
    merged_dlps.sort(
        key=lambda x: (-x["score"], 0 if x.get("source") == "user-defined" else 1)
    )

    # 选取主 DLP 和备选 DLP
    primary_dlp = merged_dlps[0] if merged_dlps else None
    secondary_dlp = merged_dlps[1] if len(merged_dlps) > 1 else None

    return {
        "merged_scored_dlps": merged_dlps,
        "primary_dlp": primary_dlp,
        "secondary_dlp": secondary_dlp,
        "custom_dlp_count": len(scored_custom_dlps),
        "builtin_dlp_count": len(scored_builtin_dlps)
    }
```
...
```yaml
custom_dlp_retrieval:
  # ---- 自定义 DLP 候选池状态 ----
  custom_dlp_pool_scanned: true
  user_dlps_dir: "rendering-pipeline/user-dlps/"
  custom_dlps_found: 2                    # 扫描到的有效自定义 DLP 数量
  invalid_dlps_skipped: 0                 # 因验证失败被跳过的数量

  # ---- 自定义 DLP 打分结果 ----
  scored_custom_dlps:
    - dlp_name: "DLP-user-techblog"
      dlp_family: "custom-publication"    # 自定义族名（custom- 前缀）
      score: 0.92
      source: "user-defined"
      file_path: "rendering-pipeline/user-dlps/DLP-user-techblog.md"
      match_details:
        - scenario_tag: "技术博客"
          match_type: "exact"
          matched_keyword: "技术"
        ...
        - scenario_tag: "开发者文档"
          match_type: "partial"
          matched_keyword: "开发工具"
          score: 0.5
    - dlp_name: "DLP-user-academic-cn"
      dlp_family: "academic-journal"      # 复用内置族
      score: 0.75
      source: "user-defined"
      file_path: "rendering-pipeline/user-dlps/DLP-user-academic-cn.md"
      match_details: [...]
  ...
  merged_ranking:
    - dlp_name: "DLP-user-techblog"
      score: 0.92
      source: "user-defined"              # 自定义 DLP 排名第一
    - dlp_name: "DLP-linear"
      score: 0.85
      source: "builtin"                   # 内置 DLP 排名第二
    - dlp_name: "DLP-economist"
      score: 0.78
      source: "builtin"
    - dlp_name: "DLP-user-academic-cn"
      score: 0.75
  ...
  primary_dlp:
    dlp_name: "DLP-user-techblog"
    score: 0.92
    source: "user-defined"                # 主 DLP 为自定义 DLP
    selection_reason: "得分最高（0.92），自定义 DLP 优先"
  secondary_dlp:
    dlp_name: "DLP-linear"
    score: 0.85
    source: "builtin"                     # 备选 DLP 为内置 DLP
```

### 11.6 自定义 DLP 检索完整流程伪代码

```python
# ============================================================
# DLP 检索器 — 自定义 DLP 检索完整流程
# 在主流程 §八 的 Step 4 之后插入自定义 DLP 检索步骤
# ============================================================

function retrieve_with_custom_dlps(
    uir_document,
    explicit_task_type=None,
    user_preference=None
):
    """
    扩展主流程：在内置 DLP 打分后，合并自定义 DLP 打分结果
    """

    # Step 1-3: 复用主流程的语义信号提取、任务类型映射、族预筛选
    semantic_signals = extract_semantic_signals(uir_document)
    task_type_mapping = map_task_type(semantic_signals, explicit_task_type)
    target_family = task_type_mapping["primary_family"]

    # Step 4a: 内置 DLP 族内打分（复用主流程）
    builtin_scoring_result = score_dlps_in_family(
        target_family=target_family,
        semantic_signals=semantic_signals,
        task_type_mapping=task_type_mapping
    )

    # Step 4b: 自定义 DLP 候选池扫描与打分（新增）
    custom_dlp_pool = scan_custom_dlp_pool("rendering-pipeline/user-dlps/")

    scored_custom_dlps = score_custom_dlps(
        custom_dlp_pool=custom_dlp_pool,
        semantic_signals=semantic_signals,
        task_type_mapping=task_type_mapping
    )

    # Step 4c: 合并内置 DLP 与自定义 DLP 打分结果（新增）
    merged_result = merge_and_rank_dlps(
        scored_builtin_dlps=builtin_scoring_result["scored_dlps"],
        scored_custom_dlps=scored_custom_dlps
    )

    # Step 5: 质量保持策略（复用主流程，但传入合并后的结果）
    exhaust_retry_result = apply_exhaust_retry_strategy(
        scoring_result=merged_result,
        task_type_mapping=task_type_mapping,
        semantic_signals=semantic_signals
    )

    # Step 6-7: 适配器输出与最终组装（复用主流程）
    final_dlp_name = exhaust_retry_result["final_dlp"]["dlp_name"]
    final_dlp_object = get_dlp_by_name_or_path(
        final_dlp_name,
        custom_dlp_pool  # 同时查找内置和自定义 DLP
    )
    design_tokens = adapt_dlp_to_design_tokens(final_dlp_object)

    # 组装最终输出（含自定义 DLP 检索信息）
    dlp_retriever_output = {
        "matched_dlp": final_dlp_name,
        "matched_family": exhaust_retry_result["final_dlp"]["dlp_family"],
        "match_score": exhaust_retry_result["final_dlp"]["score"],
        "matched_dlp_source": "user-defined" if any(
            d["dlp_name"] == final_dlp_name and d.get("source") == "user-defined"
            for d in merged_result["merged_scored_dlps"]
        ) else "builtin",
        "exhaust_retry_used": exhaust_retry_result["exhaust_retry_used"],
        "design_tokens": design_tokens,
        "custom_dlp_retrieval": {
            "custom_dlp_pool_scanned": len(custom_dlp_pool) > 0,
            "custom_dlps_found": len(custom_dlp_pool),
            "scored_custom_dlps": scored_custom_dlps,
            "merged_ranking": merged_result["merged_scored_dlps"]
        },
        "confidence": exhaust_retry_result["final_confidence"]
    }
...
function get_dlp_by_name_or_path(dlp_name, custom_dlp_pool):
    """
    根据 DLP 名称查找 DLP 对象
    优先在自定义 DLP 候选池中查找，找不到再查找内置 DLP
    """
    # 先在自定义 DLP 候选池中查找
    for custom_dlp in custom_dlp_pool:
        if custom_dlp["name"] == dlp_name:
            return custom_dlp

    # 再在内置 DLP 中查找
    return get_dlp_by_name(dlp_name)
```

### 11.7 自定义 DLP 检索与主流程的集成点

自定义 DLP 检索在主流程（§八）中的集成点如下：

| 主流程步骤 | 集成动作 | 说明 |
|-----------|---------|------|
| Step 1: 语义信号提取 | 无变化 | 自定义 DLP 检索复用相同的语义信号 |
| Step 2: 任务类型映射 | 无变化 | 自定义 DLP 检索复用相同的任务类型映射 |
| Step 3: 族预筛选 | 无变化 | 自定义 DLP 若属于内置族则归入对应族，若为 custom-* 族则独立打分 |
| Step 4: 族内打分 | **新增 Step 4b/4c** | 在内置 DLP 打分后，扫描自定义 DLP 候选池并合并打分结果 |
| Step 5: 质量保持策略 | 无变化 | 自定义 DLP 不可用时回退到内置 DLP 质量保持策略 |
| Step 6: 适配器输出 | **扩展查找逻辑** | `get_dlp_by_name` 扩展为 `get_dlp_by_name_or_path`，同时查找内置和自定义 DLP |
| Step 7: 组装最终输出 | **新增 custom_dlp_retrieval 字段** | 输出中增加自定义 DLP 检索信息 |

### 11.8 自定义 DLP 检索的穷尽重试策略

| 穷尽重试路径 | 触发条件 | 行为 |
|-------------|---------|------|
| user-dlps/ 目录不存在 | 首次使用时目录未创建 | 跳过自定义 DLP 检索，仅使用内置 DLP，标注 `custom_dlp_pool_scanned=false` |
| user-dlps/ 目录为空 | 目录存在但无 .md 文件 | 跳过自定义 DLP 检索，标注 `custom_dlps_found=0` |
| 自定义 DLP 文件 YAML 解析失败 | frontmatter 格式错误 | 跳过该文件，记录到 `invalid_dlps_skipped`，继续扫描其他文件 |
| 自定义 DLP 12 字段不完整 | 缺失必填字段 | 跳过该文件，记录验证错误，继续扫描其他文件 |
| 自定义 DLP 未通过 ASR 硬门验证 | 入库时未验证或验证失败 | 该 DLP 不进入候选池（入库前验证为前置条件） |
| 自定义 DLP 场景标签为空 | `applicable_scenarios` 为空列表 | 跳过该文件，记录错误 `applicable_scenarios 必须为非空列表` |

---

> **知识来源**: Visual DNA 审美进化项目 / brand-identity-skill 元规则 / Taste-Skill 全局审美总控 / UIR v2.0 全息框架 §1-§8 / design-language-profiles 16 个 DLP / R6-04 自定义 DLP 入口
