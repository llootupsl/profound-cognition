...
> **机器可读规则源 (R6-03)**: 所有 44 条 ASR 规则的完整定义（含 rationale / severity / override_condition 字段）已写入 [`asr-rules.yaml`](../asr-rules.yaml)（项目根目录）。本文件为人类可读说明 + 执行逻辑，asr-rules.yaml 为机器可读规则源，两者保持同步。ASR 检查器应优先读取 asr-rules.yaml 进行自动化检测。
...
### ASR-IMAGE-001: 禁 AI 生图 API 生成图片直接使用

| 字段 | 内容 |
|------|------|
| **检测规则** | 双重检测：(1) 图片元数据检测：EXIF/metadata 含 AI 生成标记（`Software: Midjourney`/`Software: DALL-E`/`Software: Stable Diffusion`/`Software: Flux`/`Software: Qwen-Image`/`generator` 字段）；(2) 代码扫描检测：渲染产物中含任何 AI 生图 API 调用代码（`flux`/`stable_diffusion`/`qwen_image`/`dall_e`/`midjourney`/`comfyui`/`replicate`/`stability_ai` 等 API 调用）。任一检测命中即违规。 |
| **违规示例** | (1) 直接嵌入 Midjourney 生成的图片，未做任何后处理；(2) 渲染代码中调用 `flux.generate_image()` 或 `qwen_image.text_to_image()` 等 AI 生图 API |
| **修复建议** | **完全禁用 AI 生图 API**。所有图像必须通过代码生成（SVG / Mermaid / Canvas / CSS / Typst draw）。替换为内联 SVG（见 illustration-generator.md §1）或 Mermaid 代码块（见 illustration-generator.md §2）。详见 [illustration-generator.md §0 核心铁律](../output/illustration-generator.md) 的 `forbidden_apis` 清单。 |
| **来源标注** | Impeccable 禁用清单驱动理念·配图风格统一约束 + illustration-generator.md §0 核心铁律（代码生成优先，禁用 AI 生图 API） |
...
### 9.2 违规清单格式（R6-03 透明化）

> **透明化要求**: 每条违规输出必须包含 `rationale`（设计理由）和 `override_condition`（豁免条件），确保规则可解释、可申诉。规则字段完整定义见 [`asr-rules.yaml`](../asr-rules.yaml)。
...
[ASR-XXXX-NNN] 禁令描述
  ...
  ├─ 检测规则: <触发的检测规则说明>
  ├─ 修复建议: <具体的修复方案>
  ├─ 设计理由: <rationale — 为什么要有这条规则>
  ├─ 严重等级: <blocking|warning>
  └─ 豁免条件: <override_condition — 何种情况下可豁免>
```
...
| 字段 | 来源 | 说明 |
|------|------|------|
| 违规代码 | 运行时检测 | 检测到的具体代码片段或色值 |
| 检测规则 | asr-rules.yaml `detection_rule` | 触发的检测规则说明 |
| 修复建议 | asr-rules.yaml `fix_suggestion` | 具体的修复方案 |
| 设计理由 | asr-rules.yaml `rationale` | **新增 (R6-03)** — 为什么要有这条规则，帮助开发者理解规则意图 |
| 严重等级 | asr-rules.yaml `severity` | **新增 (R6-03)** — `blocking`（违反即拒）/ `warning`（警告但允许输出） |
| 豁免条件 | asr-rules.yaml `override_condition` | **新增 (R6-03)** — 满足此条件时规则不触发，支持合理豁免 |
...
[ASR-FONT-001] 禁 Inter 作为 Premium 产出字体
  ...
  ├─ 设计理由: Inter 是 LLM 训练数据统计平均的产物，被广泛用作'安全字体'，缺乏设计意图与审美主张。Premium 产出必须有意识地选择具备独特字怀和笔画特征的字体，以体现设计决策。
  ├─ 严重等级: blocking
  └─ 豁免条件: 当产出类型为系统 UI 组件库文档（output_type=ui_documentation）且需与宿主应用字体保持一致时，允许使用 Inter 作为 fallback 字体

[ASR-COLOR-001] 禁 AI 紫 #7C3AED 系
  ├─ 违规代码: --color-primary: #7C3AED;
  ├─ 检测规则: 色值在 #7C3AED ±10% 范围内 (H=263°, S=84%, L=58%)
  ├─ 修复建议: 替换为学术蓝 #1A56DB 或暖调人文 #B45309
  ├─ 设计理由: AI 紫 #7C3AED 是 LLM 生成内容最显著的'统计平均色'，一眼可辨为 AI 产物。该色相饱和度过高，缺乏品牌个性，且与多数品牌色系冲突。
  ├─ 严重等级: blocking
  └─ 豁免条件: 当品牌方官方品牌色即为紫色系（如 Twitch #9146FF / Yahoo #6001D2）且提供 brand_guideline 时允许使用

门禁结果: 拒绝输出（2 条 blocking 违规 + 0 条 warning 警告）
重试状态: 已计入熔断计数（当前 1/3）
```
...
| severity | 处理方式 |
|----------|---------|
| `blocking` | 违反即拒，返回违规清单，触发穷尽重试（计入熔断计数） |
| `warning` | 记录警告日志，允许输出但标记待复审，不计入熔断计数 |

> **豁免条件评估**: 当违规代码命中 `override_condition` 时，该违规降级为 `warning`（即使原 severity 为 blocking），并在违规清单中标注 `[OVERRIDE-APPLIED]`。豁免评估需提供可验证的证据（如 brand_guideline 引用、output_type 声明等）。

### 9.3 熔断机制

| 熔断状态 | 触发条件 | 行为 |
|---------|---------|------|
| 正常 | 未触发质量驱动终止（`consecutive_low_improvement < 2`） | 返回违规清单，触发重试 |
| 熔断 | 触发质量驱动终止（`consecutive_low_improvement >= 2`，即连续 2 次重试分数提升 < 1 分） | 质量保持为最高分方案（详见 fuse-mechanism.md），ASR 硬门在质量保持方案上仍执行核心禁令检查（ASR-COLOR-001/002 + ASR-FONT-001），确保质量保持方案不违反最基础的审美底线 |

> **与 fuse-mechanism.md 一致性说明**: 本表熔断条件"`consecutive_low_improvement >= 2`"与 fuse-mechanism.md 伪代码 `while True` + 质量驱动终止检测（连续 2 次重试分数提升 < 1 分即终止，无硬性重试次数上限，符合 EXHAUST 模式四大铁律）完全一致。质量保持策略统一为"选择最高分方案"，ASR 硬门在此质量保持方案上额外执行核心禁令检查（ASR-COLOR-001/002 + ASR-FONT-001），确保质量保持方案不违反最基础的审美底线。
