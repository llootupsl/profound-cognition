...
>
> 追求五维门禁全部 100 分，采用质量驱动终止（连续 2 次重试分数提升 < 1 分即终止），超过则质量保持为最高分方案。不设硬性重试次数上限——与 EXHAUST 模式四大铁律一致。
...
function fuse_mechanism(render_output):
    ...
    scores_history = []
    outputs_history = []
    consecutive_low_improvement = 0  # 质量驱动终止计数器

    while True:  # 无硬性上限，由质量驱动终止
        attempt += 1
        ...
        // Step 5: 质量驱动终止检测（连续 2 次重试分数提升 < 1 分）
        if len(scores_history) >= 2:
            current_score = sum(scores_history[-1])
            previous_score = sum(scores_history[-2])
            improvement = current_score - previous_score
            if improvement < 1:
                consecutive_low_improvement += 1
            else:
                consecutive_low_improvement = 0  # 重置计数器
            if consecutive_low_improvement >= 2:
                // 连续 2 次重试分数提升 < 1 分，触发质量保持
                return exhaust_retry(scores_history, outputs_history)

        // Step 6: 定向修复（仅修复未满分维度，含可回滚原则 §2.3）
...
- **无硬性上限保证**：`while True` 循环不设重试次数上限，与 EXHAUST 模式「永远穷尽无档位无上限」铁律一致。终止仅由质量驱动条件决定（全满分或连续 2 次重试分数提升 < 1 分）。
- **质量驱动终止**：当连续 2 次重试的综合得分提升均 < 1 分时，判定为"质量收敛"，触发质量保持。这确保重试不会在质量仍有显著提升空间时过早终止，也不会在质量已收敛时空耗资源。
...
> **EXHAUST 一致性声明**：本节原 v1 版本采用"最大重试 3 次"硬上限，与 EXHAUST 模式四大铁律（Token 不设上限 / 时间不设限制 / 质量唯一优先 / 永远穷尽无档位无上限）冲突。v2 已移除硬上限，改为质量驱动终止（连续 2 次重试分数提升 < 1 分即终止），与 §1 算法伪代码 `while True` + `consecutive_low_improvement` 计数器完全一致。

### 3.1 质量驱动终止计数器

| 计数器 | 含义 | 说明 |
| --- | --- | --- |
| `attempt` | 渲染尝试序号 | 从 1 开始递增，记录进入五维门禁审查的次数，不设上限 |
| `scores_history` | 五维得分历史 | 累积每次通过 ASR + Golden Set 后的五维得分，供质量保持挑选最优 |
| `outputs_history` | 渲染输出历史 | 累积每次通过 ASR + Golden Set 后的渲染输出，与 `scores_history` 一一对应 |
| `consecutive_low_improvement` | 连续低提升计数器 | 当连续 2 次重试的综合得分提升 < 1 分时，触发质量保持，终止重试 |

> **质量驱动终止语义说明**: 伪代码中 `while True` 循环不设重试次数上限。终止仅由两种条件决定：(1) 任一 `attempt` 达到全满分 → 立即 PASS；(2) `consecutive_low_improvement >= 2`（即连续 2 次重试分数提升 < 1 分）→ 触发质量保持。这与 §1 算法伪代码 `while True` + Step 5 质量驱动终止检测完全一致。

### 3.2 计数规则

- **ASR 硬门失败触发重试**：修复后重新进入循环（不计入 `scores_history`，因未通过 ASR）
- **Golden Set 校验失败触发重试**：修复后重新进入循环（不计入 `scores_history`，因未通过 Golden Set）
- **五维门禁未满分触发重试**：得分与输出计入 `scores_history` / `outputs_history`，并更新 `consecutive_low_improvement` 计数器
- **L1-L5 渲染层重试**：**不计入**熔断计数（详见第 5 节）

### 3.3 终止条件

- 任一 `attempt` 达到全满分 → 立即 PASS，终止流程
- `consecutive_low_improvement >= 2`（连续 2 次重试分数提升 < 1 分）且仍未全满分 → 进入质量保持流程，终止重试
- 不存在基于重试次数硬上限的终止分支（`while True` 无退出条件，仅由质量驱动条件终止）
...
当 `consecutive_low_improvement >= 2`（即连续 2 次重试的综合得分提升均 < 1 分）时，判定为"质量收敛"，仍未全满分则触发质量保持。此条件由 §1 算法伪代码 Step 5 检测，确保重试不会在质量仍有显著提升空间时过早终止，也不会在质量已收敛时空耗资源。
...
L1-L5 渲染层重试（技术重试，独立计数）
   │
   ▼ 渲染成功
熔断机制（审美重试，质量驱动终止，无硬性上限）
   │
   ├─ Step 1: ASR 硬门 ──失败──► 修复 ──► 重试（不计入 scores_history）
   │                                  │
   │                                  └─ 连续 2 次提升 < 1 分 ──► 质量保持
   │
   ├─ Step 2: Golden Set 校验 ──失败──► 修复 ──► 重试（不计入 scores_history）
   │                                        │
   │                                        └─ 连续 2 次提升 < 1 分 ──► 质量保持
   │
   ├─ Step 3: 五维门禁 ──未满分──► 定向修复 ──► 重试（计入 scores_history）
   │                                              │
   │                                              └─ 连续 2 次提升 < 1 分 ──► 质量保持
   │
   └─ 全满分 ──► PASS（进入导出）
...
五维门禁 ──审美门禁 95 分，其余满分（综合 480/500）──►
   ...
   │  consecutive_low_improvement = 0（首次，无前序对比）
   ▼
...
ASR 硬门 ──通过──► Golden Set 校验 ──通过──►
   │
   ▼
五维门禁 ──审美门禁 95.5 分，其余满分（综合 480.5/500）──►
   │  scores_history = [..., {排版:100, 审美:95.5, 配图:100, 语义:100, 品牌:100}]
   │  outputs_history = [..., render_output_v2]
   │  improvement = 480.5 - 480 = 0.5 < 1 → consecutive_low_improvement = 1
   ▼
定向修复审美门禁（换一种修复策略）
   │
   ▼
...
ASR 硬门 ──通过──► Golden Set 校验 ──通过──►
   │
   ▼
五维门禁 ──审美门禁 96 分，其余满分（综合 481/500）──►
   │  scores_history = [..., {排版:100, 审美:96, 配图:100, 语义:100, 品牌:100}]
   │  outputs_history = [..., render_output_v3]
   │  improvement = 481 - 480.5 = 0.5 < 1 → consecutive_low_improvement = 2
   ▼
consecutive_low_improvement >= 2，判定质量收敛，仍未全满分 ──► 触发质量保持
   │
   ▼
exhaust_retry(scores_history, outputs_history)
   │
   ▼
3 次尝试综合得分：480/500, 480.5/500, 481/500
   │
   ▼
最高分方案：第 3 次尝试（render_output_v3，481/500）
   │
   ▼
标注：[FUSE-EXHAUST-RETRY] 未达满分，已质量保持为最高分方案（综合得分 481/500）
   │
   ▼
EXHAUST_RETRY（render_output_v3, 481）
```

**结果**：连续 2 次重试的综合得分提升均 < 1 分（480→480.5→481，每次提升 0.5 分），判定为质量收敛，触发质量保持。从 `scores_history` 中挑选综合得分最高的第 3 次尝试方案（481/500），添加质量保持标注后输出。注意：此处 attempt=3 仅为本场景示例的实际尝试次数，并非硬上限——若提升仍显著（≥ 1 分），重试会继续进行。
...
1. **终止性**：`fuse_mechanism` 必在以下两种条件之一满足时终止，无死循环风险：(a) 任一 `attempt` 达到全满分；(b) `consecutive_low_improvement >= 2`（连续 2 次重试分数提升 < 1 分）。这两种条件必然在有限步内满足其一（因五维得分有上界 500，且连续低提升计数器单调递增直至触发阈值）。
...
5. **计数独立性**：L1-L5 渲染层重试不计入熔断的质量驱动终止计数（`consecutive_low_improvement` 仅由五维门禁得分历史驱动）。
