...
- **触发节点**: Phase 1 上下文组装阶段（T00 完成后、T02 启动前）
- **触发阈值**: `context_package` 预估 token 数 ≥ 12000
- **平台要求**: 支持 Python 3.9+ 且 `pip install llmlingua` 成功
- **降级激活**: 当 Python 环境不可用时，自动降级为 LLM-only 规则驱动摘要（不安装 Python 包）

---

## 错误处理

- **LLMLingua 安装失败**: 降级为规则驱动摘要，保留 key_findings 前 N 条 + main_conclusion
- **压缩后 token 仍超限**: 触发 Level 3 aggressive 压缩，仅保留 core_conclusion + 前 3 条 key_findings
- **压缩后信息丢失过多**: 在 execution_ledger 中记录 `compression_info_loss_warning`，供 Supervisor 检查
- **Python 执行环境不可用**: 自动切换到 `exhaust-retry_mode`（LLM-only 规则驱动摘要）
