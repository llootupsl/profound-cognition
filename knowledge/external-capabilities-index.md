...
> - maintainer: 阿洋
> - changelog:
>   - v1.0 初始版本（86+ 张能力卡逆向索引）
>   - v1.1 补全版本治理元数据与交叉引用（D12.4.2-D12.4.3）

## 交叉引用

- **上游**: `knowledge/cognitive-framework.md`（认知流水线）
- **下游**: 全部 DAG 节点（按需消费能力卡）
- **相关**: `knowledge/external-capabilities/`（86+ 张能力卡文件）、`knowledge/domain-engines.md`（领域引擎索引）、`plugins/`（插件适配器）
- **协议**: `protocols/execution-protocol.md`（节点执行时按需加载能力卡）
...
| TC-022 | AutoFigure | T27 | 图表自动生成 | **deprecated** — 原依赖 LLM 推理后端，已改为代码生成（Mermaid/SVG/Observable Plot） |
| LC-023 | Markmap | T27 | 思维导图渲染 | ★核心方法论已内化于 rendering-pipeline/ARCHITECTURE.md
| TC-024 | PubFig | T27 | 学术图表标准 | **deprecated** — 原依赖 LLM 推理后端，已改为代码生成（Matplotlib/SVG/Typst draw） |
...
| TC-043 | PaperBanana | T02 | 论文解析 | **deprecated** — 原依赖 Google 专有 API（Nano Banana Pro/Gemini 3 Pro Image），已改为代码生成（内联 SVG/Mermaid/Canvas） |
| TC-044 | PaperVizAgent | T20a | 论文可视化 | **deprecated** — 原依赖专有 API，已改为代码生成（Observable Plot/ECharts/内联 SVG） |
...
| LC-033 | PaperBanana-Skill | T27 | 顶刊级学术插图生成 | **deprecated** — 原依赖 Google 专有 API（Nano Banana Pro/Gemini 3 Pro Image），已改为代码生成（内联 SVG/Mermaid/Canvas）。★核心方法论已内化于 rendering-pipeline/visual-dna.md |
...
| 卡片编号 | 卡片名称 | 状态 | 潜在整合方向 |
|---------|---------|------|-------------|
| TC-005 | Mem0 | **已弃用**（迁移至 #5b Mem0.md v6.0 增强版；见 [TC-005-Mem0.md](external-capabilities/TC-005-Mem0.md) 头部 deprecated 标注） | 记忆存储层 |
...
| TC-103 | OpenScholar | 保留待扩展 | 学术文献搜索 |
| TC-104 | Tongyi-DeepResearch | 保留待扩展 | 深度研究引擎 |
| TC-105 | GraphRAG | 保留待扩展 | 知识图谱 |
| TC-106 | RAGFlow | 保留待扩展 | RAG 引擎 |
| TC-107 | Self-Refine | 保留待扩展 | 认知综合 |
| TC-108 | OpenFactCheck | 保留待扩展 | 事实核查 |
| TC-109 | RAGAS | 保留待扩展 | RAG 评估 |
| TC-110 | Marker | 保留待扩展 | PDF 解析 |
| TC-111 | Docling | 保留待扩展 | 文档解析 |
| TC-112 | ColPali | 保留待扩展 | 多模态检索 |
| TC-113 | Instructor | 保留待扩展 | 结构化输出 |
| TC-114 | OpenResearcher | 保留待扩展 | 深度研究 |
| TC-115 | nano-graphrag | 保留待扩展 | 轻量级 RAG |
| TC-116 | dodiscover | 保留待扩展 | 因果发现 |
| TC-117 | CrewAI | 保留待扩展 | 多智能体 |
| TC-118 | AutoGen | 保留待扩展 | 多智能体 |
| TC-119 | Factiverse | 保留待扩展 | 事实核查 |
| TC-120 | scholarly | 保留待扩展 | 学术搜索 |
| TC-121 | proplot | 保留待扩展 | 科学图表 |
| TC-122 | LanceDB | 保留待扩展 | 向量数据库 |
| TC-123 | Promptfoo | 保留待扩展 | LLM 评估 |
| TC-124 | UQLM | 保留待扩展 | 不确定性量化 |
| TC-125 | Quarto | 保留待扩展 | 可复现文档 |
| TC-126 | tigramite | 保留待扩展 | 因果发现（时序） |
| TC-127 | tree-of-thought | 保留待扩展 | 多路径推理 |
| TC-128 | Chroma | 保留待扩展 | 向量数据库 |
| TC-129 | Reflexion | 保留待扩展（Audit-7 Stage 2 补建） | I01 迭代深化增强 |
| TC-130 | Semantic Scholar API | 保留待扩展（Audit-7 Stage 2 补建） | T02 研究底座检索 |
| TC-131 | OpenAlex API | 保留待扩展（Audit-7 Stage 2 补建） | T02 研究底座补充检索 |
| TC-132 | SciencePlots | 保留待扩展（Audit-7 Stage 2 补建） | T27 可视化渲染样式 |
...
> **铁律**: 本类别下所有"领域配图"必须通过代码生成（SVG / Mermaid / Canvas / CSS），**禁止使用任何 AI 生图 API**（Flux / SD / Qwen-Image / DALL-E / Midjourney）。详见 [illustration-generator.md §0 核心铁律](../output/illustration-generator.md)。

| 技术名称 | 状态 | 消费节点/处置说明 |
|---------|------|-----------------|
| SDXL | **deprecated** | ❌ 已废弃——Stable Diffusion 3.5 穷尽重试链违反"代码生成优先"原则，已被 [illustration-generator.md §0 核心铁律](../output/illustration-generator.md) 明确列入 `forbidden_apis` 清单。所有原 SDXL 消费场景（T27 配图）改为代码生成图（内联 SVG / Mermaid / Observable Plot） |
| MONAI | **deprecated** | 医学影像专用框架，当前全息框架 31 域中 health-engine 已覆盖医学分析，不依赖 MONAI 生成影像 |
| 金融领域配图 | **激活**（代码生成） | illustration-generation-protocol §6（13步子代理工作流，Mermaid/SVG 自足方式）；消费节点 T27 |
| 历史领域配图 | **激活**（代码生成） | illustration-generation-protocol §6（13步子代理工作流，Mermaid/SVG 自足方式）；消费节点 T27 |
| 科学领域配图 | **激活**（代码生成） | illustration-generation-protocol §6（13步子代理工作流，Mermaid/SVG 自足方式）+ PubFig 学术图表（matplotlib paper style）；消费节点 T27 |
| 设计领域配图 | **激活**（代码生成） | illustration-generation-protocol §6（13步子代理工作流，Mermaid/SVG 自足方式）+ aesthetic-enhancer §10.4 创意主题；消费节点 T27 |
...
- **保留待扩展**: 43 张（含 TC-103~TC-128 共 26 张 Wave 6 补录 + TC-129~TC-132 共 4 张 Audit-7 Stage 2 补建）
- **方法论引用（无独立文件）**: 39 项
- **借鉴技术激活**: 10 项
- **借鉴技术 deprecated**: 9 项
- **总计**: 222 项卡片 + 19 项借鉴技术