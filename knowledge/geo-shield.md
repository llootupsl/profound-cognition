...
- **上游**: `knowledge/cognitive-framework.md`（认知流水线）、`knowledge/research-methods.md`（研究方法）
- **下游**: `tasks/T01_input_triage.md`（输入分流器调用偏见扫描）、`tasks/T18_quality_bias.md`（偏见检测）
- **相关**: `knowledge/sensitivity-framework.md`（敏感度分级，与偏见检测结果联动）、`knowledge/object-router.md`（对象路由器，同为 T01 子模块）、`knowledge/ethics-references.md`（伦理标准）
