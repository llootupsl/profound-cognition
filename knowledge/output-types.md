...
> - maintainer: 阿洋
> - changelog:
>   - v1.0 初始版本（research_report/wechat_article/course_material 三种类型）
>   - v1.1 补全版本治理元数据与交叉引用（D12.4.2-D12.4.3）

## 交叉引用

- **上游**: `knowledge/cognitive-framework.md`（认知流水线）
- **下游**: `tasks/T00_outline.md`（大纲生成，按 output_type 路由）、`tasks/T20_output_guard.md`（输出守卫）、`tasks/T20a_research_render.md`、`tasks/T20b_wechat_render.md`、`tasks/T20c_course_render.md`
- **相关**: `knowledge/article-archetypes.md`（公众号文章原型，仅 wechat_article 触发）、`knowledge/typography-guide.md`（排版方法论，按 output_type 选择引擎）、`output/rendering-tech-stack.md`（渲染技术栈）
- **协议**: `protocols/output-expansion-protocol.md`（输出扩展）、`protocols/output-rendering-protocol.md`（输出渲染）、`protocols/multi-form-delivery-protocol.md`（多形态交付）
...
- 消费全部节点产出（含Phase 5元维度引擎 T22-T28）
