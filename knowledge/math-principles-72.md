...
> - maintainer: 阿洋
> - changelog:
>   - v1.0 初始版本（72 项数学原理覆盖追踪）
>   - v1.1 补全版本治理元数据与交叉引用（D12.4.2-D12.4.3）

## 交叉引用

- **上游**: `knowledge/cognitive-framework.md`（认知流水线）
- **下游**: `tasks/TM01_system_dynamics.md`、`tasks/TM02_causal_verification.md`、`tasks/TM03_adversarial_synthesis.md`、`tasks/TM04_scenario_landscape.md`、`tasks/TM05_meta_reflection.md`、`tasks/TM06_meta_layer_verify.md`、`tasks/TM07_ontology_export.md`
- **相关**: `knowledge/thinking-models/decision/bayesian-updating.md`、`knowledge/thinking-models/decision/game-theory.md`、`knowledge/thinking-models/decision/scenario-simulator.md`、`knowledge/thinking-models/general/systems-thinking.md`、`formula-engine/`（公式引擎实现）、`docs/formula-call-chain-map.md`（公式调用链图）
...
## 公式引擎与数学原理编号对齐（D12.4.1）

> 本节建立 `formula-engine/` 下 4 个非线性公式（FE-001 ~ FE-004）与 72 项数学原理的显式编号映射关系，确保公式实现可追溯至底层数学原理。

### 编号映射表

| 公式编号 | 公式名称 | 公式文件 | 对应数学原理编号 | 数学原理名称 | 关联说明 |
|---------|---------|---------|----------------|------------|---------|
| FE-001 | Softmax 动态注意力加权 | [formula-engine/softmax-attention.md](../formula-engine/softmax-attention.md) | MP-024 | 信息论 (Information Theory) | Softmax 是信息熵最大化的分布（最大熵原理） |
| FE-001 | Softmax 动态注意力加权 | 同上 | MP-025 | 熵 (Entropy) | 温度参数 T 控制输出分布的熵 |
| FE-001 | Softmax 动态注意力加权 | 同上 | MP-026 | KL 散度与互信息 (KL Divergence) | Softmax 输出与均匀分布的 KL 散度衡量分化程度 |
| FE-002 | Logistic 胜负判定函数 | [formula-engine/logistic-adjudication.md](../formula-engine/logistic-adjudication.md) | MP-003 | 非线性动力学 (Nonlinear Dynamics) | Logistic 函数是典型非线性 S 型曲线 |
| FE-002 | Logistic 胜负判定函数 | 同上 | MP-024 | 信息论 (Information Theory) | P(win) 可视为攻击信息传递效率 |
| FE-002 | Logistic 胜负判定函数 | 同上 | MP-041 | 前景理论 (Prospect Theory) | 0.7/0.3 阈值反映认知不对称性 |
| FE-003 | 指数边际收益衰减模型 | [formula-engine/info-decay.md](../formula-engine/info-decay.md) | MP-024 | 信息论 (Information Theory) | ΔInfo(t) 是信息增益度量，ε 是收敛阈值 |
| FE-003 | 指数边际收益衰减模型 | 同上 | MP-023 | 随机过程 (Stochastic Processes) | 指数衰减是随机过程中常见收敛模式 |
| FE-003 | 指数边际收益衰减模型 | 同上 | MP-049 | 动态规划 (Dynamic Programming) | 迭代深化决策基于动态规划思想 |
| FE-004 | Sigmoid 置信度校准函数 | [formula-engine/sigmoid-calibration.md](../formula-engine/sigmoid-calibration.md) | MP-003 | 非线性动力学 (Nonlinear Dynamics) | Sigmoid 是非线性 S 型映射 |
| FE-004 | Sigmoid 置信度校准函数 | 同上 | MP-041 | 前景理论 (Prospect Theory) | 中间拉伸/极端压缩符合概率加权特征 |
| FE-004 | Sigmoid 置信度校准函数 | 同上 | MP-050 | 凸优化 (Convex Optimization) | Sigmoid 是凸优化经典激活函数 |

### 编号对齐说明

1. **MP 编号体系**：本表中 `MP-XXX` 为 72 项数学原理的统一编号（MP-001 ~ MP-072），对应上方各章节中的序号（如 MP-003 = 第 3 项"非线性动力学"，MP-024 = 第 24 项"信息论"）
2. **FE 编号体系**：`FE-001` ~ `FE-004` 为公式引擎的统一编号，与 `docs/formula-call-chain-map.md` 中的编号一致
3. **多对多关系**：一个公式可对应多个数学原理（如 FE-001 对应 MP-024/025/026），一个数学原理也可被多个公式引用（如 MP-024 被 FE-001/002/003 引用）
4. **双向追溯**：每个公式文件末尾含「数学原理交叉引用」章节指向本表，本表指向公式文件，形成双向可追溯链路

### 覆盖统计

- 4 个公式共覆盖 **8 个不同的数学原理**（MP-003/023/024/025/026/041/049/050）
- 覆盖类别：系统与动力学（1）、概率与统计（3）、博弈论与决策（1）、优化（1）、机器学习（1，间接）
- MP-024（信息论）被 3 个公式引用，是最核心的数学基础
