...
> **职责边界**：本文档为搜索策略的知识层定义（规则+标准+量化表）。
> 具体执行策略（聚合流程、质量评分公式、去重算法）详见 strategies/search-aggregation.md。
> 查询重写策略详见 strategies/query-rewriting.md。
> 三角验证策略详见 strategies/triangulation.md。
