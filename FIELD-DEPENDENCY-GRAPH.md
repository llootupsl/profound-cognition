...
NRSF-Full（叙事研究状态文件 —— 全量笔记）
  ...
  │
  ├─→ Phase 7: 元维度引擎（T22-T28）—— 仅 research_report
  ...
  │
  ├─→ Phase 7: 科学层（TM01-TM07）—— 仅 research_report + system_dynamics_required
  ...
  │     ├─→ TM04（场景规划 EMA+CIB+CLA+3Horizons）消费 TM03 完成 → 产出 → TM04 输出
  │     ├─→ TM05（元认知反思 MetaNet+ENA+Cynefin）消费 TM04 完成 → 产出 → TM05 输出
  │     ├─→ TM06（14 维 + 元维度扩展验证）消费 TM05 完成 → 产出 → TM06 输出
...
## 3. Phase 7 节点定义速查（从 SKILL.md DAG 派生）
...
| T28 | Gate-终 最终质量门控（6项检查） | NRSF-Full §T27 | 门控判定（PASS/FAIL） |
...
| TM04 | 场景规划（EMA+CIB+CLA+3Horizons） | TM03 输出 | TM04 输出 |
| TM05 | 元认知反思（MetaNet+ENA+Cynefin） | TM04 输出 | TM05 输出 |
| TM06 | 14 维 + 元维度扩展验证 | TM05 输出 | TM06 输出 |
| TM07 | 知识图谱导出（OWLAPY+SSSOM+PyKEEN+Neo4j） | TM06 输出 | TM07 输出 |
| T_gate_delta | Gate-δ 科学层门控 | TM07 + T_philosophical_core + T_meta_dim 节点 | 门控判定（PASS/FAIL） |
...
Gate-δ（科学层门控） → T_gate_delta
  │
  ├─→ 检查对象：TM01-TM07 + T_philosophical_core + T_meta_dim 节点
  ├─→ 检查维度：
...
见 SKILL.md DAG 拓扑定义（57 节点完整 YAML 块）。
