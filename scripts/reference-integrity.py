...
# -*- coding: utf-8 -*-
# 作者：阿洋
"""reference-integrity.py — Profound Cognition v6.0.0 参考完整性校验脚本
...
def parse_dag_nodes(skill_md_path):
    ...
    # 跳过 YAML 模板示例占位符（如 "节点 ID"、"{T09}"、"{T10}"），仅接受标准节点 ID
    # 标准节点 ID：以字母/下划线开头，只含字母数字/下划线，不含花括号/中文字符/空格
    VALID_NODE_ID_RE = re.compile(r'^[A-Za-z_][A-Za-z0-9_]*$')
    ...
    for block in node_blocks:
        ...
        # 跳过模板示例占位符（含花括号/中文/空格等非标准字符）
        if not VALID_NODE_ID_RE.match(node_id):
            continue
