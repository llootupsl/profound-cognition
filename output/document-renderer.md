...
> **统一错误处理遵循**：本穷尽重试管线遵循 `rendering-pipeline/ARCHITECTURE.md` §渲染管线统一错误处理（R9-03）的 RERR-RENDER-* 错误码与 5 层格式适配策略。各 level 的失败映射如下：
> - level_1_weasyprint 失败 → `RERR-RENDER-001`（渲染引擎不可用）/ `RERR-RENDER-002`（渲染超时，超过 P95×3）/ `RERR-RENDER-004`（输出格式错误）
> - level_2_pandoc / level_3_html / level_4_markdown 失败 → 沿用 `RERR-RENDER-*` 错误码，按 Layer 2-4 格式适配链路降级
> - level_5_plain_text 输出对应 Layer 4 纯文本输出（内容完整保留，标注为穷尽重试渲染输出）
> - 全部失败 → Layer 5 错误报告（结构化错误码 + 失败原因 + 格式适配链路），记录到 execution_ledger

```yaml
exhaust_retry_pipeline:
  error_code_compliance: "RERR-RENDER-001/002/003/004 (see ARCHITECTURE.md R9-03)"
  level_1_weasyprint:
