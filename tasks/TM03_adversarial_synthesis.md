...
## 与 T10/T11/T12 的分工明确化（R4-03）

TM03 与 T10/T11/T12 同为对抗节点，但分工不同——T10/T11/T12 负责**路径级对抗**，TM03 负责**综合级对抗**。两者不得重复攻击。

### 分工边界

| 维度 | T10/T11/T12（路径级对抗） | TM03（综合级对抗） |
|------|---------------------------|---------------------|
| **攻击对象** | 单条推理路径的结论 | T13 综合后的整体结论（多路径融合后的涌现结论） |
| **攻击视角** | 逻辑漏洞（T10）/ 证据缺口（T11）/ 范围越界（T12） | 涌现性攻击 / 一致性攻击 / 完备性攻击（三新维度） |
| **攻击层级** | 路径级——检验单条推理过程是否有谬误 | 综合级——检验多路径融合后的整体结论是否有系统性缺陷 |
| **执行时机** | T13 之前——对 T09 多路径推理的每条路径独立攻击 | T13 之后——对 T13 综合后的整体结论进行元对抗 |
| **依赖关系** | 依赖 T09_summary | 依赖 T13 综合产出（含 T10/T11/T12 的路径级攻击结果） |

### TM03 三新维度定义

TM03 在 T10/T11/T12 路径级对抗之上，新增三个综合级对抗维度：

```yaml
tm03_new_dimensions:
  emergence_attack:
    description: "涌现性攻击——检验 T13 综合结论是否产生了单条路径无法预测的涌现属性"
    check_items:
      - "综合结论是否出现了单路径未提及的新属性（涌现属性）？这些涌现属性是否有充分支撑？"
      - "综合结论是否忽略了单路径中的重要属性（涌现缺失）？"
      - "多路径融合是否产生了协同效应或冲突效应？这些效应是否被正确识别？"
    difference_from_t10: "T10 检验单路径逻辑漏洞；涌现性攻击检验多路径融合后的新属性是否合理"

  consistency_attack:
    description: "一致性攻击——检验 T13 综合结论在不同路径之间是否自洽"
    check_items:
      - "T13 综合结论是否与所有支撑路径的结论一致？是否存在路径A支持但路径B反对的结论被综合为'成立'？"
      - "T13 综合结论的置信度是否与各路径的置信度匹配？是否存在高置信度路径被低置信度路径拉低的情况？"
      - "T13 综合结论是否在不同抽象层级上自洽？高层结论与底层证据是否一致？"
    difference_from_t11: "T11 检验单路径证据缺口；一致性攻击检验多路径之间的结论一致性"

  completeness_attack:
    description: "完备性攻击——检验 T13 综合结论是否覆盖了所有应覆盖的维度"
    check_items:
      - "T13 综合结论是否覆盖了 T09 所有路径的关键洞察？是否有路径被遗漏？"
      - "T13 综合结论是否覆盖了 T10/T11/T12 攻击中发现的所有重要脆弱性？是否有脆弱性被忽略？"
      - "T13 综合结论是否覆盖了所有必要的分析维度（实证/比较/预测/诊断/评估等）？"
    difference_from_t12: "T12 检验单路径范围越界；完备性攻击检验综合结论的覆盖完整性"
```

### TM03 读取 T10/T11/T12 结果避免重复攻击

TM03 在执行综合级对抗前，必须读取 T10/T11/T12 的路径级攻击结果，避免重复攻击：

```yaml
tm03_read_upstream:
  upstream_sources:
    - source: "T10_output.logic_attacks"
      purpose: "了解路径级逻辑攻击已覆盖的漏洞，避免 TM03 重复攻击单路径逻辑问题"
    - source: "T10_output.uncovered_vulnerabilities"
      purpose: "识别 T10 未覆盖的逻辑漏洞，作为 TM03 涌现性攻击的候选目标"
    - source: "T11_output.evidence_attacks"
      purpose: "了解路径级证据攻击已覆盖的缺口，避免 TM03 重复攻击单路径证据问题"
    - source: "T11_output.evidence_supplement_needed"
      purpose: "识别证据补充建议，作为 TM03 一致性攻击的候选目标"
    - source: "T12_output.scope_attacks"
      purpose: "了解路径级范围攻击已覆盖的越界，避免 TM03 重复攻击单路径范围问题"
    - source: "T12_output.failure_boundaries"
      purpose: "识别失效边界，作为 TM03 完备性攻击的候选目标"
    - source: "T12b_output.cross_adversarial_assessment"
      purpose: "了解三路对抗交叉融合结果，避免 TM03 重复 T12b 已发现的内部矛盾"
    - source: "T12b_output.meta_adversarial_review"
      purpose: "了解元对抗审查结果，作为 TM03 综合级对抗的输入"

  deduplication_rule: |
    TM03 在执行涌现性/一致性/完备性攻击时，必须先检查该攻击是否已被 T10/T11/T12 覆盖：
    - 若已被 T10/T11/T12 覆盖 → 跳过，标注 [DUPLICATE_OF_{T10|T11|T12}]
    - 若部分被覆盖 → 仅执行未被覆盖的部分，标注 [PARTIAL_DUPLICATE]
    - 若完全未被覆盖 → 执行 TM03 综合级攻击，标注 [TM03_ORIGINAL]

  deduplication_log:
    description: "去重日志，记录 TM03 跳过/执行的所有攻击"
    fields:
      - attack_id: "TM03 攻击标识"
      - dimension: "emergence_attack|consistency_attack|completeness_attack"
      - status: "[TM03_ORIGINAL]|[DUPLICATE_OF_{T10|T11|T12}]|[PARTIAL_DUPLICATE]"
      - upstream_reference: "若为 DUPLICATE，引用 T10/T11/T12 的对应攻击"
```
...
- T13 的交叉融合产出
- T08 的认知解构结果
- **T10/T11/T12 的路径级攻击输出（R4-03 分工明确化）**
- **T12b 的元对抗审查输出（R3-05）**
...
adversarial_synthesis:
  ...
  # R4-03 综合级对抗三新维度
  emergence_attack:
    description: "涌现性攻击——检验综合结论的涌现属性"
    findings:
      - finding: "涌现属性/涌现缺失描述"
        target: "受影响的综合结论"
        severity: "HIGH|MEDIUM|LOW"
        has_support: bool  # 涌现属性是否有充分支撑
  consistency_attack:
    description: "一致性攻击——检验综合结论的跨路径自洽性"
    findings:
      - finding: "不一致描述"
        path_a_position: "路径A的立场"
        path_b_position: "路径B的立场"
        synthesis_position: "T13综合后的立场"
        severity: "HIGH|MEDIUM|LOW"
  completeness_attack:
    description: "完备性攻击——检验综合结论的覆盖完整性"
    findings:
      - finding: "覆盖缺失描述"
        missing_dimension: "被遗漏的维度/路径/脆弱性"
        severity: "HIGH|MEDIUM|LOW"
  deduplication_log:
    description: "R4-03 去重日志，记录 TM03 跳过/执行的所有攻击"
    entries:
      - attack_id: "TM03 攻击标识"
        dimension: "emergence_attack|consistency_attack|completeness_attack"
        status: "[TM03_ORIGINAL]|[DUPLICATE_OF_{T10|T11|T12}]|[PARTIAL_DUPLICATE]"
...
- [ ] 【R4-03】emergence_attack / consistency_attack / completeness_attack 三新维度是否全部执行？
- [ ] 【R4-03】deduplication_log 是否记录了所有攻击的去重状态（TM03_ORIGINAL / DUPLICATE / PARTIAL_DUPLICATE）？
- [ ] 【R4-03】是否读取了 T10/T11/T12 的路径级攻击结果以避免重复攻击？
