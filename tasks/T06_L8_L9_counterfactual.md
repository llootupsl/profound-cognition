...
向 PaperQA2 索引提问，获取相关论文段落和引用，支撑 L8 反事实推演与 L9 知识边界划定：

1. **触发条件**：T02 已构建 PaperQA2 索引且 PaperQA2 服务可用
2. **查询流程**：
   - 基于 T06 的反事实前提验证需求构造查询（如"{前提改变} 的历史先例与学术讨论"）
   - 向 PaperQA2 索引提问，获取相关论文段落和引用
   - 将论文中的反事实先例、理论推演注入 `L8_counterfactual_scenarios`
3. **查询输出使用**：
   - 论文中的反事实先例 → 增强 `L8_counterfactual_scenarios[].branching_timeline`（引用学术先例）
   - 论文中的理论推演 → 增强 `L8_counterfactual_scenarios[].alternative_outcome`
   - 论文中的知识边界讨论 → 增强 `L9_knowledge_boundary.known_unknowns`
4. **穷尽重试策略**：PaperQA2 不可用 → 回退到 SearXNG 学术引擎策略 + 人工筛选

### 自检清单新增项
- [ ] 若 T02 已构建 PaperQA2 索引，是否向 PaperQA2 查询了反事实前提的学术先例？
- [ ] PaperQA2 查询结果是否已注入 `L8_counterfactual_scenarios` 和 `L9_knowledge_boundary`？
