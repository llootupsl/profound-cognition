...
causal_verification:
  ...
  # R9-04 新增字段（Task 5.23.4）
  causal_effect_estimates: "list[causal_effect_estimates] — DoWhy 因果效应估计结果（见 dowhy_estimation 子步骤）"
  robustness_checks: "list[robustness_check] — DoWhy 反驳检验结果（见 dowhy_estimation 子步骤）"
```

## dowhy_estimation 子步骤（R9-04 / Task 5.23）

> **子步骤目的**：将 DoWhy 因果推断的四步流程（假设→识别→估计→反驳）固化为 TM02 的标准化子步骤，形成可审计的因果效应估计闭环。本子步骤整合 MC-054（四步法）、TC-057（识别工具方法论）和源码逻辑引入，输出标准化的 `causal_effect_estimates` 和 `robustness_check` 字段。

### 子步骤 1：假设（Model — 因果建模）

**输入**：T09 因果图、T22 系统变量集合、TM01 反馈回路
**任务**：将领域知识编码为 DoWhy CausalModel，声明变量角色
**输出**：CausalModel 对象

```python
from dowhy import CausalModel

def dowhy_model(t09_causal_graph, t22_system_variables, hypothesis):
    """DoWhy 四步流程 - 步骤 1：因果建模"""
    model = CausalModel(
        data=data_or_pseudo_dataset,
        ...
        mediators=hypothesis.get("mediators", []),
        graph=t09_causal_graph_to_dot(t09_causal_graph)
    )
    return model
```
...
### 子步骤 2：识别（Identify — 因果效应可识别性判定）

**输入**：CausalModel 对象
**任务**：使用 do-calculus 判断因果效应是否可识别，选择识别策略
**输出**：IdentifiedEstimand 对象

```python
def dowhy_identify(model):
    """DoWhy 四步流程 - 步骤 2：识别"""
...
**识别策略决策树**：
1. 优先尝试后门准则（backdoor）→ 寻找最小调整集
2. 后门调整集不可观测 → 尝试前门准则（frontdoor）
3. 后门和前门均不满足 → 尝试工具变量法（IV）
4. 所有策略均不满足 → 标注 UNIDENTIFIABLE，穷尽重试为敏感性分析

### 子步骤 3：估计（Estimate — 因果效应计算）

**输入**：IdentifiedEstimand 对象 + 观测数据（或 T09 伪数据集）
**任务**：选择估计方法，计算 ATE/CATE/ITE
**输出**：CausalEstimate 对象

```python
def dowhy_estimate(model, identified_estimand, need_cate=False):
    """DoWhy 四步流程 - 步骤 3：估计"""
    # ATE 估计（DoWhy 原生）
    ...
    if need_cate:
        try:
            ...
            # EconML 不可用时穷尽重试为定性异质性分析
            cate_estimate = None

    return ate_estimate, cate_estimate
```
...
| 条件 | 方法 |
|------|------|
| 后门策略 + 调整集小 + 数据大 | `backdoor.linear_regression` |
| 后门策略 + 倾向性评分可行 | `backdoor.propensity_score_weighting` (IPW) |
| 前门策略 | `frontdoor.linear_regression` (链式分解) |
| IV 策略 | `iv.instrumental_variable` (Wald/2SLS) |
| 需要 CATE 异质性 | `backdoor.econml.dml.DoublyLearner` (EconML 后端) |
| 无实证数据 | 使用 T09 伪数据集，标注 `pseudo_data_warning=true` |

### 子步骤 4：反驳（Refute — 稳健性验证）

**输入**：CausalEstimate 对象 + 观测数据
**任务**：执行多种鲁棒性检验
**输出**：RefutationResult 列表 + robustness_check 字段

```python
def dowhy_refute(model, identified_estimand, estimate):
    """DoWhy 四步流程 - 步骤 4：反驳"""
    refutation_results = []

    # 1. 安慰剂处理检验
    placebo = model.refute_estimate(identified_estimand, estimate, method_name="placebo_treatment_refuter")
    refutation_results.append({"method": "placebo_treatment", "result": placebo})

    # 2. 随机公共原因检验
    random_cc = model.refute_estimate(identified_estimand, estimate, method_name="random_common_cause")
    refutation_results.append({"method": "random_common_cause", "result": random_cc})

    # 3. 数据子集检验
    subset = model.refute_estimate(identified_estimand, estimate, method_name="data_subset_refuter")
    refutation_results.append({"method": "data_subset", "result": subset})
...
#### causal_effect_estimates 字段（Task 5.23.4）
...
#### robustness_check 字段（Task 5.23.4）
...
robustness_check:
  ...
  refutation_tests:
    - method: "placebo_treatment | random_common_cause | data_subset | dummy_outcome"
      ...
      effect_change: "float — 效应量变化幅度"
      p_value: "float — 反驳检验 p 值"
      passed: "bool — 是否通过反驳检验"
      pass_criteria: "通过判据描述"
  robustness_summary:
    ...
    level_criteria:
      ...
      uncertain: "不可识别或伪数据集"
  sensitivity_analysis:
    e_value: "float | null — E-value"
    confounding_robustness: "HIGH | MEDIUM | LOW"
    interpretation: "E-value 解读"
  retry_status:
    level: "FULL | PARTIAL_A | PARTIAL_B | RETRYING"
    degradation_note: "string | null — 降级说明"
```

### EconML CATE 估计后端（Task 5.23.5）

> **架构声明**：EconML（TC-058）保留为 DoWhy 的 CATE 估计后端。DoWhy 是因果推断主引擎，负责建模→识别→估计→反驳四步流程；EconML 专注于异质性处理效应（CATE）估计，通过 DoWhy 的 `method_name="backdoor.econml.dml.*"` 参数集成。
...
| 识别 | ✅ 后门/前门/IV | ❌ |
| ATE 估计 | ✅ 回归/IPW/2SLS | ❌ |
| CATE 估计 | 🔗 调用 EconML | ✅ DoublyLearner/CausalForestDML |
| 反驳 | ✅ Placebo/Subset/Dummy | ❌ |
| 敏感性 | 🔗 调用 EconML E-value | ✅ E-value 计算 |

**穷尽重试策略**：当 EconML 不可用时，按 L1→L2→L3→L4 逐级穷尽重试：
- L1：EconML 完整 CATE 估计（DoublyLearner/CausalForestDML）
- L2：DoWhy 原生 ATE 估计（无 CATE 异质性分析）
- L3：定性异质性分析（基于理论推理识别效应异质性方向）
- L4：纯文字因果描述（无数值估计）
