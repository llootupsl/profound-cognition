...
| 导出格式 | 用途 | 说明 |
|---------|------|------|
| OWL/RDF XML | 本体交换 | OWL 2 标准序列化格式 |
| JSON-LD | Web 语义 | 支持链接数据的 JSON 格式 |
| Cypher 脚本 | 图数据库 导入 | 包含节点、关系、索引、约束 |
| GraphML | Gephi 可视化 | 支持图形可视化工具导入 |
| Mermaid 图 | Markdown 嵌入 | 支持在文档中直接渲染知识图谱 |
...
- [ ] 循环定义是否已检测？
- [ ] 多格式导出是否完整（OWL/RDF + JSON-LD + Cypher + GraphML + Mermaid）？
- [ ] 图数据库 环境依赖是否已标注？
- [ ] 本体版本号是否为 v5.1.0？
- [ ] 穷尽重试状态是否与实际执行情况一致？
