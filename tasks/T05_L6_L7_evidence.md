...
向 PaperQA2 索引提问，获取相关论文段落和引用，支撑 L6 证据账本构建：

1. **触发条件**：T02 已构建 PaperQA2 索引且 PaperQA2 服务可用
2. **查询流程**：
   - 基于 T05 的证据验证需求构造查询（如"{主张} 的实证证据"）
   - 向 PaperQA2 索引提问，获取相关论文段落和引用
   - 将论文中的实证证据、对立证据注入 `L6_evidence_ledger`
3. **查询输出使用**：
   - 论文中的实证证据 → 增强 `L6_evidence_ledger[].support_strength`（论文为 L0 级来源，support_strength ≥ 0.7）
   - 论文中的对立证据 → 增强 `L6_evidence_ledger[].counter_evidence`
   - 论文来源标注为 `source_level: L0`（同行评审论文）
4. **穷尽重试策略**：PaperQA2 不可用 → 回退到 SearXNG 学术引擎策略 + 人工筛选

### 自检清单新增项
- [ ] 若 T02 已构建 PaperQA2 索引，是否向 PaperQA2 查询了证据账本中核心主张的论文证据？
- [ ] PaperQA2 查询结果是否已注入 `L6_evidence_ledger` 并标注 `source_level: L0`？
