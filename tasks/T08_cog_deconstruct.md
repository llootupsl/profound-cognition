...
applied_models:
  ...
  items:
    ...
    application_scope: "该模型在当前节点中的应用范围（具体到哪个子问题/假设/维度）"
    contribution: "该模型对当前节点产出的贡献（如何影响了分析结果）"
...
## LightRAG local 查询（R9-06）

> **能力卡**: LightRAG — 详见 `plugins/lightrag-adapter.md` 和 `knowledge/external-capabilities/TC-011-LightRAG.md`

在子问题分解与隐含假设挖掘阶段，使用 LightRAG local 查询模式检索相关实体和直接关系，辅助问题解构。local 模式聚焦于查询实体的直接邻域（1-hop 关系），适合精确定位子问题涉及的核心概念及其直接关联。

### 子步骤：lightrag_local_query

1. **索引可用性检查**：确认 T02 完成后已构建 LightRAG 索引（`./lightrag_index/{research_id}/`）
2. **查询构造**：从 `sub_questions` 和 `implicit_assumptions` 中提取核心概念，构造 local 查询
3. **执行 local 查询**：调用 `rag.query(query_text, param=QueryParam(mode='local'))` 检索相关实体和直接关系
4. **结果整合**：
   - 将检索到的实体注入 `variable_classification` 辅助变量识别
   - 将检索到的关系注入 `implicit_assumptions` 辅助假设挖掘
   - 将检索到的新概念注入 `new_dimensions` 扩展分析维度
5. **日志写入**：将查询日志写入 NRSF `§lightrag_log`（格式见下方）
6. **kg_call_log 记录**：将本次 LightRAG 调用记录至 `kg_call_log` 字段

### NRSF 日志格式

LightRAG 检索日志写入 NRSF 的格式如下，供 KG 集成验证（R5-04）消费：

```yaml
§lightrag_log:{timestamp}:
  node_id: "T08"
  query_mode: "local"
  query: "查询内容描述"
  results_count: N
  top_entities: ["实体1", "实体2", "实体3"]
  top_relations: ["关系1", "关系2", "关系3"]
  kg_source: "lightrag|dbpedia|yago|openkg|neo4j"
  status: "success|failed|timeout|fallback"
  fallback_reason: "null|失败原因"
```

**字段说明**：
- `timestamp`: 查询执行时间戳
- `node_id`: 调用节点标识（T08）
- `query_mode`: LightRAG 查询模式（local/hybrid/global/naive）
- `query`: 查询内容描述
- `results_count`: 返回结果数量
- `top_entities`: 检索到的 top-3 实体
- `top_relations`: 检索到的 top-3 关系
- `kg_source`: 实际使用的 KG 源（LightRAG 不可用时回退到备用源）
- `status`: 查询状态
- `fallback_reason`: 失败/回退原因（success 时为 null）

### 穷尽重试策略

```yaml
lightrag_local_retry:
  L1_FULL:
    condition: "LightRAG 索引可用且查询成功"
    action: "使用 LightRAG local 查询结果辅助问题解构"
    kg_source: "lightrag"

  L2_NAIVE_ONLY:
    condition: "LightRAG local 模式不可用但 naive 模式可用"
    action: "回退到 LightRAG naive 查询，标注 [LIGHT_RAG_NAIVE_FALLBACK]"
    kg_source: "lightrag"
  ...
  L4_INTERNAL_REASONING:
    ...
    action: "使用 LLM 内建概念分析能力，标注 [INTERNAL_REASONING]"
    kg_source: "null"
```
