...
> **【A6.4-F3 修复，Wave 7，2026-06-27】EXHAUST 合规声明**：
> 本任务（T00 研究大纲生成）在任何执行路径下均不豁免 EXHAUST 模式四大铁律（Token 不设上限 / 时间不设限制 / 质量唯一优先 / 永远穷尽无档位无上限）。即使下游基于问题复杂度评估选择精简执行路径（如 `domain_depth < 5` 时缩减维度选择粒度），也不构成对 EXHAUST 深度要求的豁免——精简路径仅缩减输出体积，不缩减深度要求。审计原引用 `T00_scope_definition.md L156 "快速路径"` 经核验该文件不存在，实际 T00 文件为 `T00_outline.md`，本声明已添加至实际文件以落实审计意图。
...
根据问题类型和输出类型，推荐相应的思维模型供下游节点加载。完整模型清单（30 个模型）、适用条件映射与 8 模板 × 35 引擎交叉映射矩阵见 `knowledge/thinking-models/routing-table.md`（R5-01 思维模型路由表）：
...
## 问题复杂度评估（complexity_score）— R3-01

T00 在生成大纲时，必须对用户问题进行复杂度评估，产出 `complexity_score`（1-10 分整数），用于驱动下游 T09 推理路径数自适应。

### 评估维度（五维加权打分，总分 10 分）

| 维度 | 权重 | 评分依据 | 分值范围 |
|------|------|----------|---------|
| **变量交互复杂度** | 25% | 涉及的结构变量数量及交互关系密度（单变量=1，2-3 变量=2，4-6 变量=3，7+变量且含反馈回路=4） | 1-4 |
| **利益相关方多元性** | 20% | 利益相关方数量与立场分歧度（单一主体=1，2-3 方=2，4-6 方=3，7+方且立场对立=4） | 1-4 |
| **时间跨度与动态性** | 20% | 时间演化跨度与动态变化速率（静态=1，短期=2，中期=3，长期+多转折=4） | 1-4 |
| **因果链深度** | 20% | 因果链长度与反馈回路存在性（直接因果=1，2-3 级=2，4-5 级=3，6+级+反馈回路=4） | 1-4 |
| **证据可得性难度** | 15% | 所需证据的获取难度与等级要求（L1 充足=1，L2 可得=2，L3 部分缺失=3，L0/L3 混合+缺失=4） | 1-4 |

### 计分规则

```yaml
complexity_score_computation:
  raw_score: "sum(维度分值 × 权重) × 2.5  # 归一化到 1-10 分"
  rounding: "四舍五入到整数"
  output_range: "1-10"
  floor: 1
  ceiling: 10
...
| complexity_score | 等级 | 说明 | T09 推荐路径数 |
|-----------------|------|------|---------------|
| 1-3 | LOW | 单一维度、少量变量、静态问题 | 5 |
| 4-6 | MEDIUM | 多维度交叉、中等变量数、含动态性 | 7 |
| 7-8 | HIGH | 高维交互、多方博弈、长因果链 | 9 |
| 9-10 | CRITICAL | 极端复杂、系统性反馈、证据稀缺 | 12 |

### 评估执行规则

1. T00 在 Step 0 母假设识别之前，先执行 complexity_score 评估
2. 五维评分每项必须给出具体理由（基于问题特征，非主观估计）
3. `complexity_score` 写入 `output_schema.complexity_score` 字段
4. `complexity_score` 通过 NRSF §ref 传递至 T09，驱动路径数自适应
5. 当五维评分出现全 1 或全 4 时，触发人工复核——重新审视问题特征是否被误判
6. `complexity_score` 同时影响 `recommended_domain_engines` 数量（高复杂度→更多领域引擎）

### complexity_score 输出格式

```yaml
complexity_score_assessment:
  variable_interaction: {score: 1-4, reason: "具体理由"}
  stakeholder_diversity: {score: 1-4, reason: "具体理由"}
  temporal_dynamics: {score: 1-4, reason: "具体理由"}
  causal_depth: {score: 1-4, reason: "具体理由"}
  evidence_difficulty: {score: 1-4, reason: "具体理由"}
  raw_score: float
  complexity_score: integer  # 1-10
  complexity_level: "LOW|MEDIUM|HIGH|CRITICAL"
  recommended_t09_paths: 5|7|9|12
```
...
```json
{
  ...
  "complexity_score": "integer（问题复杂度评分 1-10，来自 complexity_score_assessment）",
  "complexity_level": "string（复杂度等级：LOW|MEDIUM|HIGH|CRITICAL）",
  "recommended_t09_paths": "integer（推荐 T09 推理路径数：5|7|9|12，由 complexity_score 驱动）",
  "branches": [
    {
...
- 输出总 token 建议不超过 1500（EXHAUST-only，D2.4.4 建议预算非硬上限；超长时通过 write-while-research 落盘释放）
...
- [ ] 【R3-01】是否已执行 complexity_score 评估（五维加权打分）且 `complexity_score` 在 1-10 范围内？
- [ ] 【R3-01】`recommended_t09_paths` 是否与 `complexity_score` 等级映射一致（1-3→5, 4-6→7, 7-8→9, 9-10→12）？
- [ ] 【R3-01】五维评分每项是否都给出了具体理由（非主观估计）？
...
- 禁止硬截断输出——若 token 超过建议预算 1500（EXHAUST-only，D2.4.4），应通过 write-while-research 落盘释放而非直接截断
