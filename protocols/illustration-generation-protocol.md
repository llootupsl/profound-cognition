...
illustration_types:
  ...
  photos_and_realia:
    - type: "photo_realia"
      ...
      supported_formats: [png_base64, json_description]
      complexity: [low, medium]
      backend_tools:
        primary: [image_retrieval_api, qwen_image]
        description: "Image retrieval API / Qwen-Image（标注为AI生成）"
...
| 可选服务增强 | 外部论文制图服务 | PNG / SVG | 需外部 API |

> **PaperBanana 专有 API 说明**: PaperBanana 的 Visualizer 使用 Nano Banana Pro（Gemini 3 Pro Image），为 Google 专有 API。开源环境不可用，AutoFigure 为纯开源备选优先使用。PaperBanana 仅在专有 API 可用环境下使用。
...
## 7. PaperBanana 5 智能体流水线

> **来源技能**: PaperBanana Skill
> **融入目的**: 全类型学术配图质感统一，通过 5 智能体流水线确保配图质量闭环

融入 PaperBanana Skill 的 5 智能体流水线，用于全类型学术配图质感统一：

```yaml
paperbanana_pipeline:
  description: |
    PaperBanana 5 智能体流水线，用于全类型学术配图质感统一。
  ...
  flow: "analyze → plan → generate → review → refine"
  retry_policy: "review 不通过时，回流至 generate 重新生成，穷尽重试直至 review 通过"
```

### 7.1 与 §6 论文框架图工作流的关系

PaperBanana 5 智能体流水线是 §6 论文框架图生成工作流的**质量增强层**：

- §6 的 P5（第一轮候选图）→ 对应 PaperBanana generate 智能体
...
> **注意**: PaperBanana 的 Visualizer 使用 Nano Banana Pro（Gemini 3 Pro Image），为 Google 专有 API。开源环境不可用时，回落至 §6.4 默认 Mermaid/SVG 自足方式，绝不省略配图。
...
| PaperBanana 流水线 | PaperBanana 5-Agent Pipeline | 5 智能体（analyze/plan/generate/review/refine）学术配图质量闭环流水线 |
...
- [§7 PaperBanana 5 智能体流水线](#7-paperbanana-5-智能体流水线) — 来源：PaperBanana Skill，5 智能体学术配图质量闭环
